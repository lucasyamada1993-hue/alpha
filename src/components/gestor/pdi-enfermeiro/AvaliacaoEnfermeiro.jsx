...
import { pdiEnfermeiroStore } from "@/lib/pdiEnfermeiroStore";
...
export default function AvaliacaoEnfermeiro({
  ...
  const [competencias] = useState(() => pdiEnfermeiroStore.getActiveByCategory());

  const ENFERMEIROS = [
    "Dra. Paula Teixeira",
    "Enfermeiro João Silva",
    "Enfermeira Maria Santos",
    "Enfermeiro Pedro Oliveira",
    "Enfermeira Carla Dias",
    "Ana Souza",
  ];
  ...
  const { data: historico = [] } = useQuery({
    queryKey: ["pdi_historico_enfermeiro", colaborador],
    queryFn: () =>
      colaborador
        ? db.entities.PDIEnfermeiro.filter(
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => db.entities.PDIEnfermeiro.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pdi_historico_enfermeiro"] });
  ...
  const handleSubmit = (e) => {
    ...
    // @ts-expect-error — payload alinhado a PDIEnfermeiro.create; tipagem genérica do mutate
  ...
  const calcularEstatisticas = (avaliacoes) => {
    ...
    const mediaAtual = calcMedia(mesAtual.competencias);
    const mediaAnterior = mesAnterior ? calcMedia(mesAnterior.competencias) : mediaAtual;
            ...
            {historico.map((avaliacao, idx) => (
              <div key={avaliacao.id} className="border-t border-gray-100 first:border-t-0">
                ...
                {expandedHistorico === avaliacao.id && (
                  <div className="px-5 py-4 bg-gray-50 border-t border-gray-100 text-sm space-y-3">
                    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                disabled={modoEmbutido && Boolean(colaboradorInicialProp)}
              ...
              {modoEmbutido && colaboradorInicialProp && (
